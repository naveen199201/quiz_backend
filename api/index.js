  ...
  try {
    ...
    if (categorizeQuestions.length > 0) {
      const categorizePromises = categorizeQuestions.map((question) => {
        ...
        if (question._id.length===24) {
          // If the question has an _id, update it
          return Categorize.findByIdAndUpdate(question._id, question, {
            new: true,
          });
        } else {
          // If the question does not have an _id, create a new question
    ...
    if (comprehensionQuestions.length > 0) {
      const comprehensionPromises = comprehensionQuestions.map((question) => {
        console.log(question._id)
        if (question._id.length===24) {
          return Comprehension.findByIdAndUpdate(question._id, question, {
            new: true,
          });
        } else {
          delete question._id;
    ...
    res
      .status(500)
      .json({ error: "An error occurred while saving or updating questions." });
  }
});
...
app.get("/api/questions", async (req, res) => {
  ...
  try {
    ...
    const categorizeQuestions = await Categorize.find({}, categorizeParams);
...
app.post("/api/remove", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: `An error occurred while deleting the ${type} question.`,
      });
  }
});
